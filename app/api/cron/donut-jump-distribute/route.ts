// Place in: app/api/cron/donut-jump-distribute/route.ts
...
// MUST match how donut_jump_scores stores week number
...
async function runDistribution(week: number, dryRun: boolean) {
  console.log(`[Donut Jump Distribution] Starting for week ${week}, dryRun: ${dryRun}`);
  console.log(`[Donut Jump Distribution] Current week: ${getCurrentWeek()}, distributing week: ${week}`);

  // Check if already distributed for this week
  const { data: existingDistribution } = await supabase
    .from("donut_jump_prize_distributions")
    .select("id")
    .eq("week", week)
    .single();
  ...
  // Get top 10 scores for the week (deduplicated by fid, best score only)
  ...
  console.log(`[Donut Jump Distribution] Found ${distributions.length} winners`);
  ...
  for (const dist of distributions) {
    try {
      ...
      console.log(`[Donut Jump Distribution] Sent ${dist.amount} SPRINKLES to rank ${dist.rank}: ${hash}`);
  ...
  const { error: insertError } = await supabase.from("donut_jump_prize_distributions").insert({
    week,
...
export async function GET(request: NextRequest) {
  ...
  if (authHeader === `Bearer ${process.env.CRON_SECRET}`) {
    try {
      ...
      console.error("[Donut Jump Distribution] Cron error:", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
  }
