// Place in: app/api/games/donut-survivors/leaderboard/route.ts
...
export async function GET(req: NextRequest) {
  console.log("[Donut Survivors] Leaderboard API called");
  ...
  try {
    ...
    console.log("[Donut Survivors] Fetching scores for week:", currentWeek);

    // Fetch more scores to ensure we get enough unique players after deduplication
    const { data: scores, error } = await supabase
      .from("donut_survivors_scores")
      .select("fid, wallet_address, username, display_name, pfp_url, score, survival_time, kills, created_at")
      .eq("week", currentWeek)
      .gt("score", 0)
    ...
    if (error) {
      console.error("[Donut Survivors] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[Donut Survivors] Got scores:", scores?.length || 0);
    ...
    for (const score of scores || []) {
      if (!seenFids.has(score.fid)) {
        ...
        leaderboard.push({
          ...
          score: score.score,
          survivalTime: score.survival_time,
          kills: score.kills,
        });
    ...
    if (fidParam) {
      ...
      if (userEntry) {
        userStats = {
          ...
          bestSurvivalTime: userEntry.survivalTime,
          bestKills: userEntry.kills,
          rank: userEntry.rank,
    ...
    console.log("[Donut Survivors] Returning leaderboard with", leaderboard.length, "entries");
    ...
    console.error("[Donut Survivors] Leaderboard error:", error);
