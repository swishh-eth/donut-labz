...
// XP progression (moderate scaling)
...
interface UpgradeOption { type: UpgradeType; weaponType?: WeaponType; gadgetType?: GadgetType; title: string; description: string; icon: string; isTranscendent?: boolean; }
...
const ENEMY_CONFIG: Record<EnemyType, { hp: number; speed: number; size: number; xpValue: number; damage: number; color: string; spawnWeight: number }> = {
  ...
  jellybean: { hp: 18, speed: 1.6, size: 14, xpValue: 3, damage: 10, color: '#FF69B4', spawnWeight: 10 },
  licorice: { hp: 45, speed: 0.6, size: 24, xpValue: 8, damage: 15, color: '#1a1a1a', spawnWeight: 5 },
  chocolate_chunk: { hp: 90, speed: 0.45, size: 32, xpValue: 12, damage: 20, color: '#A78BFA', spawnWeight: 3 },
  wind_spirit: { hp: 1, speed: 4.0, size: 18, xpValue: 0, damage: 0, color: '#87CEEB', spawnWeight: 0 },
...
interface PrizeInfo { totalPrize: number; prizeStructure: { rank: number; percent: number; amount: string }[]; }
const PRIZE_PERCENTAGES = [40, 20, 15, 8, 5, 4, 3, 2, 2, 1];
function calculatePrizeStructure(totalPrize: number) { return PRIZE_PERCENTAGES.map((percent, i) => ({ rank: i + 1, percent, amount: ((totalPrize * percent) / 100).toFixed(2) })); }
const DEFAULT_PRIZE_INFO: PrizeInfo = { totalPrize: 5, prizeStructure: calculatePrizeStructure(5) };
...
export default function DonutSurvivorsPage() {
    ...
    const fetchPrizeInfo = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setPrizeInfo({ totalPrize: data.totalPrize, prizeStructure: data.prizeStructure || calculatePrizeStructure(data.totalPrize) });
        }
      } catch (e) { console.error("Failed to fetch prize info:", e); }
    };
    fetchPrizeInfo();
  }, []);
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.userStats) { setGamesPlayedThisWeek(data.userStats.gamesPlayed || 0); setGamesPlayed(data.userStats.gamesPlayed || 0); }
      }
    } catch (error) { console.error("Failed to fetch leaderboard:", error); }
  }, [context?.user?.fid]);
  
  useEffect(() => { if (context?.user?.fid) fetchLeaderboard(); }, [context?.user?.fid, fetchLeaderboard]);
  ...
  const submitScore = useCallback(async (finalScore: number, st: number, kills: number) => {
    ...
    const checksum = Array.from(metricsString).reduce((acc, char) => ((acc << 5) - acc + char.charCodeAt(0)) | 0, 0).toString(16);
    const metrics = { gameDurationMs, survivalTimeSeconds: st, kills, xpCollected, level, weaponsAcquired, gadgetsAcquired, bossesDefeated, powerUpsCollected, damageDealt: Math.round(damageDealt), damageTaken: Math.round(damageTaken), killsPerMinute: Math.round(killsPerMinute * 100) / 100, xpPerMinute: Math.round(xpPerMinute * 100) / 100, checksum };
    try { await fetch("/api/games/donut-survivors/submit-score", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entryId, score: finalScore, survivalTime: st, kills, fid, metrics }) }); } catch (error) { console.error("Failed to submit score:", error); }
  }, []);
  ...
  const playTranscend = useCallback(() => { if (isMuted || !audioContextRef.current) return; [261.63, 329.63, 392, 523.25, 659.25, 783.99].forEach((f, i) => setTimeout(() => playSound(f, 0.4, 'sine', 0.12), i * 100)); }, [isMuted, playSound]);

  const MUSIC_STYLES = [
    { name: 'Chiptune', bpm: 180, bassNotes: [55, 65.41, 73.42, 82.41], melody: [220, 261.63, 293.66, 349.23, 392, 440, 523.25, 587.33], bassType: 'triangle' as OscillatorType, melodyType: 'square' as OscillatorType, bassVol: 0.06, melodyVol: 0.04 },
    { name: 'Sci-Fi', bpm: 200, bassNotes: [41.2, 55, 61.74, 82.41], melody: [329.63, 392, 440, 523.25, 587.33, 659.25, 783.99, 880], bassType: 'sawtooth' as OscillatorType, melodyType: 'sine' as OscillatorType, bassVol: 0.05, melodyVol: 0.05 },
    { name: 'Techno', bpm: 240, bassNotes: [55, 55, 73.42, 55], melody: [440, 493.88, 523.25, 587.33, 659.25, 698.46, 783.99, 880], bassType: 'square' as OscillatorType, melodyType: 'sawtooth' as OscillatorType, bassVol: 0.07, melodyVol: 0.035 },
    { name: 'Industrial', bpm: 220, bassNotes: [36.71, 41.2, 49, 55], melody: [196, 220, 246.94, 293.66, 329.63, 369.99, 415.3, 440], bassType: 'sawtooth' as OscillatorType, melodyType: 'square' as OscillatorType, bassVol: 0.08, melodyVol: 0.04 },
    { name: 'Chaos', bpm: 280, bassNotes: [32.7, 36.71, 41.2, 49], melody: [523.25, 554.37, 622.25, 659.25, 739.99, 783.99, 880, 932.33], bassType: 'sawtooth' as OscillatorType, melodyType: 'square' as OscillatorType, bassVol: 0.09, melodyVol: 0.045 },
  ];
  ...
  const startMusic = useCallback(() => {
    ...
    let noteIndex = 0, beatCount = 0, lastStyleCheck = 0;
    const playBeat = () => {
      if (!audioContextRef.current || !musicPlayingRef.current) return;
      const ctx = audioContextRef.current;
      ...
      if (newStyle !== musicStyleRef.current && gt - lastStyleCheck > 1) {
        ...
        lastStyleCheck = gt;
        const transOsc = ctx.createOscillator(), transGain = ctx.createGain();
        transOsc.type = 'sine';
      ...
      if (beatCount % 8 === 0) {
        ...
        bassOsc.frequency.setValueAtTime(style.bassNotes[Math.floor(beatCount / 8) % style.bassNotes.length], ctx.currentTime);
      ...
      if (beatCount % 4 === 0 || beatCount % 4 === 2) {
        const noiseLength = musicStyleRef.current >= 2 ? 0.08 : 0.05;
        const noiseGain = ctx.createGain(), noiseFilter = ctx.createBiquadFilter();
        noiseFilter.type = 'highpass';
        noiseFilter.frequency.setValueAtTime(beatCount % 4 === 0 ? 100 : 4000, ctx.currentTime);
        const bufferSize = ctx.sampleRate * noiseLength, buffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate), data = buffer.getChannelData(0);
        ...
        noiseGain.gain.setValueAtTime(beatCount % 4 === 0 ? (musicStyleRef.current >= 2 ? 0.12 : 0.08) : (musicStyleRef.current >= 2 ? 0.05 : 0.03), ctx.currentTime);
    ...
    const updateInterval = () => {
      ...
      musicIntervalRef.current = window.setInterval(() => { playBeat(); const newStyle = MUSIC_STYLES[musicStyleRef.current]; if (newStyle.bpm !== style.bpm) updateInterval(); }, intervalMs);
    };
    updateInterval();
  }, [initAudio]);
  
  const stopMusic = useCallback(() => { musicPlayingRef.current = false; if (musicIntervalRef.current) { clearInterval(musicIntervalRef.current); musicIntervalRef.current = null; } musicStyleRef.current = 0; }, []);
  
  useEffect(() => { if (isMusicOn && gameState === 'playing') startMusic(); else stopMusic(); return () => stopMusic(); }, [isMusicOn, gameState, startMusic, stopMusic]);
  ...
  const addParticles = useCallback((x: number, y: number, color: string, count: number, speed: number = 3, type?: 'spark' | 'ring' | 'glow' | 'burst') => {
    ...
    while (particlesRef.current.length + actualCount > MAX_PARTICLES) particlesRef.current.shift();
    for (let i = 0; i < actualCount; i++) {
      const a = (i / actualCount) * Math.PI * 2 + Math.random() * 0.5;
      particlesRef.current.push({ x, y, vx: Math.cos(a) * speed * (0.5 + Math.random() * 0.5), vy: Math.sin(a) * speed * (0.5 + Math.random() * 0.5), life: 1, maxLife: 1, color, size: type === 'spark' ? 1 + Math.random() * 2 : type === 'burst' ? 4 + Math.random() * 4 : 2 + Math.random() * 3, type });
    }
  }, []);
  
  const addDmgNum = useCallback((x: number, y: number, value: number, isCrit?: boolean) => { if (damageNumbersRef.current.length >= MAX_DAMAGE_NUMBERS) damageNumbersRef.current.shift(); damageNumbersRef.current.push({ x, y, value, life: 1, vy: -2, isCrit }); }, []);
  
  const rebuildGrid = useCallback(() => { enemyGridRef.current.clear(); for (const e of enemiesRef.current) { const c = getSpatialCell(e.x, e.y); if (!enemyGridRef.current.has(c)) enemyGridRef.current.set(c, []); enemyGridRef.current.get(c)!.push(e); } }, []);
  ...
  const getEnemiesNear = useCallback((x: number, y: number, radius: number): Enemy[] => {
    ...
    for (const c of cells) { const enemies = enemyGridRef.current.get(c); if (enemies) for (const e of enemies) if (!seen.has(e)) { seen.add(e); if (Math.hypot(e.x - x, e.y - y) <= radius + e.size) result.push(e); } }
    return result;
  }, []);
  ...
  const spawnEnemy = useCallback((forceType?: EnemyType) => {
    ...
    if (!forceType) {
      ...
      for (let i = 0; i < types.length; i++) { r -= weights[i]; if (r <= 0) { sel = types[i]; break; } }
    }
    const c = ENEMY_CONFIG[sel];
    const endlessMult = endlessDifficultyRef.current;
    const ts = (1 + gt / 360) * endlessMult;
    const ss = (1 + gt / 900) * Math.sqrt(endlessMult);
    enemiesRef.current.push({ x, y, type: sel, hp: Math.floor(c.hp * ts), maxHp: Math.floor(c.hp * ts), speed: c.speed * ss, size: c.size, xpValue: Math.floor(c.xpValue * endlessMult), damage: Math.floor(c.damage * endlessMult), color: c.color, hitFlash: 0, spawnAnim: 1, zigzagPhase: sel === 'jellybean' ? Math.random() * Math.PI * 2 : undefined, splitCount: sel === 'licorice' ? 2 : undefined });
  ...
  const spawnWindHorde = useCallback(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      enemiesRef.current.push({ x, y, type: 'wind_spirit', hp: c.hp, maxHp: c.hp, speed: c.speed, size: c.size, xpValue: 0, damage: 0, color: c.color, hitFlash: 0, spawnAnim: 0.5, spawnTime: now });
    }
    playWindHorde();
    ...
    weaponsRef.current.forEach(w => {
      ...
      if (w.type === 'sprinkle_shot') {
        ...
        if (nearest) {
          ...
          for (let i = 0; i < cnt; i++) { const aa = a + (i - (cnt - 1) / 2) * 0.2; projectilesRef.current.push({ x: p.x, y: p.y, vx: Math.cos(aa) * 8, vy: Math.sin(aa) * 8, damage: dmg, size: 6, color: cfg.color, piercing: Math.floor(w.level / 2), lifetime: 60, weaponType: w.type, trailTimer: 0 }); }
          addParticles(p.x, p.y, cfg.color, 2, 2, 'spark');
        }
      } else if (w.type === 'sugar_stars') {
        const cnt = 4 + w.level;
        for (let i = 0; i < cnt; i++) { const a = (i / cnt) * Math.PI * 2; projectilesRef.current.push({ x: p.x, y: p.y, vx: Math.cos(a) * 5, vy: Math.sin(a) * 5, damage: dmg, size: 8, color: cfg.color, piercing: 0, lifetime: 45, weaponType: w.type, trailTimer: 0 }); }
        addParticles(p.x, p.y, cfg.color, 6, 3, 'ring');
      } else if (w.type === 'glaze_wave') {
        const rng = 80 + w.level * 20;
        getEnemiesNear(p.x, p.y, rng).filter(e => e.type !== 'wind_spirit').forEach(e => { e.hp -= dmg; e.hitFlash = 5; addDmgNum(e.x, e.y - e.size, Math.floor(dmg)); });
        ...
        let nearest: Enemy | null = null, nd = Infinity;
        for (const e of near) { const d = Math.hypot(e.x - p.x, e.y - p.y); if (d < nd) { nearest = e; nd = d; } }
        if (nearest) { const a = Math.atan2(nearest.y - p.y, nearest.x - p.x); projectilesRef.current.push({ x: p.x, y: p.y, vx: Math.cos(a) * 6, vy: Math.sin(a) * 6, damage: dmg, size: 12 + w.level, color: cfg.color, piercing: 99, lifetime: 80, weaponType: w.type, trailTimer: 0 }); addParticles(p.x, p.y, cfg.color, 4, 3, 'spark'); }
        ...
        targets.forEach((t, i) => { const a = Math.atan2(t.y - p.y, t.x - p.x) + (i - (targets.length - 1) / 2) * 0.15; projectilesRef.current.push({ x: p.x, y: p.y, vx: Math.cos(a) * 5, vy: Math.sin(a) * 5, damage: dmg, size: 8, color: cfg.color, piercing: 0, lifetime: 120, weaponType: w.type, trailTimer: 0 }); });
  ...
  const updateOrbiting = useCallback((delta: number) => {
    ...
    weaponsRef.current.forEach(w => {
      if (w.type === 'orbiting_donuts') {
        ...
        for (let i = 0; i < cnt; i++) {
          ...
          getEnemiesNear(ox, oy, 20).filter(e => e.type !== 'wind_spirit').forEach(e => { if (e.hitFlash === 0) { e.hp -= dmg; e.hitFlash = 8; addDmgNum(e.x, e.y - e.size, Math.floor(dmg)); playHit(); if (Math.random() < 0.3) addParticles(ox, oy, '#F472B6', 2, 2, 'spark'); } });
        }
        ...
        getEnemiesNear(p.x, p.y, rad + 15).filter(e => e.type !== 'wind_spirit').forEach(e => { const d = Math.hypot(e.x - p.x, e.y - p.y); if (d < rad + 10 && d > rad - 10) { e.hp -= dmg; if (Math.random() < 0.05) addDmgNum(e.x, e.y - e.size, Math.floor(dmg * 10)); } });
      } else if (w.type === 'cinnamon_trail') {
        if (frameCountRef.current % 4 === 0) trailPointsRef.current.push({ x: p.x, y: p.y, life: 60 + w.level * 15 });
      }
    });
    ...
    if (tw) {
      ...
      trailPointsRef.current = trailPointsRef.current.filter(pt => { pt.life -= delta; getEnemiesNear(pt.x, pt.y, 18 + tw.level * 3).filter(e => e.type !== 'wind_spirit').forEach(e => { e.hp -= dmg; }); return pt.life > 0; });
    }
  }, [addDmgNum, addParticles, playHit, getEnemiesNear]);

  const checkTranscendent = useCallback(() => {
    const allWeaponsMaxed = weaponsRef.current.length === MAX_WEAPONS && weaponsRef.current.every(w => w.level >= 8);
    const allGadgetsMaxed = gadgetsRef.current.length === MAX_GADGETS && gadgetsRef.current.every(g => g.stacks >= 5);
    const canTranscend = allWeaponsMaxed && allGadgetsMaxed;
    if (canTranscend && !isTranscendent) {
      setIsTranscendent(true);
      playTranscend();
      shake(15, 500);
      flash('#FFD700', 0.4);
      addParticles(playerRef.current.x, playerRef.current.y, '#FFD700', 30, 8, 'ring');
    }
    return canTranscend;
  }, [isTranscendent, playTranscend, shake, flash, addParticles]);
  ...
  const genUpgrades = useCallback(() => {
    ...
    const canTranscend = allWeaponsMaxed && allGadgetsMaxed;
    weaponsRef.current.forEach(w => { 
      const maxLvl = canTranscend ? 99 : 8; 
      if (w.level < maxLvl && !bannedUpgrades.includes(`weapon:${w.type}`)) { 
        const c = WEAPON_CONFIG[w.type]; 
        const isTranscendentUpgrade = w.level >= 8;
        const title = isTranscendentUpgrade ? `${c.name} ★` : `${c.name} +`; 
        opts.push({ type: 'weapon', weaponType: w.type, title, description: `Lv ${w.level} → ${w.level + 1}`, icon: c.icon, isTranscendent: isTranscendentUpgrade }); 
      } 
    });
  ...
  }, [playLevelUp, shake, flash, addParticles, genUpgrades, checkTranscendent]);
  ...
  const gameLoop = useCallback(() => {
    ...
    if (sk.duration > 0) { const el = now - sk.startTime; if (el < sk.duration) { const int = sk.intensity * (1 - el / sk.duration); shakeX = (Math.random() - 0.5) * int * 2; shakeY = (Math.random() - 0.5) * int * 2; } else sk.duration = 0; }
    ...
    if (gs >= 1200 && !finalBossSpawnedRef.current) {
      ...
      shake(40, 1000); flash('#000', 0.5);
    }
    if (endlessModeRef.current && gs >= 1200) { const minutesPast20 = Math.floor((gs - 1200) / 60); endlessDifficultyRef.current = Math.pow(1.05, minutesPast20); }
    rebuildGrid(); fireWeapons(); updateOrbiting(delta);
    projectilesRef.current = projectilesRef.current.filter(proj => {
      proj.trailTimer += delta;
      if (proj.trailTimer >= 3) { proj.trailTimer = 0; if (projectileTrailsRef.current.length < 200) projectileTrailsRef.current.push({ x: proj.x, y: proj.y, life: 1, color: proj.color }); }
      if (proj.weaponType === 'mint_missiles') {
        ...
        if (nearest) { const ta = Math.atan2(nearest.y - proj.y, nearest.x - proj.x); const ca = Math.atan2(proj.vy, proj.vx); const na = ca + Math.sign(ta - ca) * Math.min(Math.abs(ta - ca), 0.15); const sp = Math.hypot(proj.vx, proj.vy); proj.vx = Math.cos(na) * sp; proj.vy = Math.sin(na) * sp; }
      }
      proj.x += proj.vx * delta; proj.y += proj.vy * delta; proj.lifetime -= delta;
      ...
      for (const e of getEnemiesNear(proj.x, proj.y, proj.size + 50)) {
        ...
        if (Math.hypot(e.x - proj.x, e.y - proj.y) < e.size + proj.size) {
          e.hp -= proj.damage; e.hitFlash = 5; addDmgNum(e.x, e.y - e.size, Math.floor(proj.damage)); playHit(); addParticles(proj.x, proj.y, proj.color, 3, 2, 'spark');
          if (proj.weaponType === 'candy_cannon') { getEnemiesNear(proj.x, proj.y, 60 + proj.size).filter(o => o.type !== 'wind_spirit').forEach(o => { if (o !== e) { o.hp -= proj.damage * 0.6; o.hitFlash = 5; } }); addParticles(proj.x, proj.y, '#A78BFA', 15, 6, 'burst'); shake(5, 100); flash('#A78BFA', 0.2); return false; }
          if (proj.weaponType === 'mint_missiles') { getEnemiesNear(proj.x, proj.y, 35).filter(o => o.type !== 'wind_spirit').forEach(o => { if (o !== e) { o.hp -= proj.damage * 0.5; o.hitFlash = 5; } }); addParticles(proj.x, proj.y, '#4ADE80', 8, 4, 'ring'); return false; }
          if (proj.piercing > 0) { proj.piercing--; proj.damage *= 0.8; } else return false;
        }
      }
      return true;
    });
    projectileTrailsRef.current = projectileTrailsRef.current.filter(t => { t.life -= 0.1; return t.life > 0; });
    ...
    enemiesRef.current = enemiesRef.current.filter(e => {
      ...
      if (!isFrozen && dist > 0) {
        let moveX = (dx / dist) * e.speed * delta, moveY = (dy / dist) * e.speed * delta;
        if (e.type === 'jellybean' && e.zigzagPhase !== undefined) { e.zigzagPhase += 0.15 * delta; const perpX = -dy / dist, perpY = dx / dist; const zigzag = Math.sin(e.zigzagPhase) * 1.5; moveX += perpX * zigzag * delta; moveY += perpY * zigzag * delta; }
        e.x += moveX; e.y += moveY;
      }
      if (e.type === 'wind_spirit') {
        if (dist < e.size + PLAYER_SIZE / 2 + 20) { const pushStrength = 6; const pushX = (p.x - e.x) / dist * pushStrength; const pushY = (p.y - e.y) / dist * pushStrength; p.vx += pushX * delta; p.vy += pushY * delta; addParticles(p.x, p.y, '#87CEEB', 2, 3, 'spark'); }
        const age = (now - (e.spawnTime || now)) / 1000;
        if (age > 10) { addParticles(e.x, e.y, '#87CEEB', 4, 2, 'spark'); return false; }
        return true;
      }
      if (e.type === 'final_boss' && dist < e.size + PLAYER_SIZE / 2) { p.hp = 0; shake(50, 1000); flash('#FF0000', 0.8); endGame(); return false; }
      if (dist < e.size + PLAYER_SIZE / 2 && Date.now() > invincibleUntilRef.current) {
        const dr = Math.min(p.defense || 0, 0.75); const ad = Math.floor(e.damage * (1 - dr));
        p.hp -= ad; invincibleUntilRef.current = Date.now() + 750 + (p.invincibilityBonus || 0);
        playHurt(); shake(10, 200); flash('#FF6B6B', 0.3); addParticles(p.x, p.y, '#FF6B6B', 8, 4);
        if (p.hp <= 0) { endGame(); return false; }
      }
      if (e.hp <= 0) {
        if (e.type === 'licorice' && e.splitCount && e.splitCount > 0) { for (let i = 0; i < 2; i++) { const angle = Math.random() * Math.PI * 2; const splitDist = 20; if (enemiesRef.current.length < MAX_ENEMIES) { enemiesRef.current.push({ x: e.x + Math.cos(angle) * splitDist, y: e.y + Math.sin(angle) * splitDist, type: 'sprinkle', hp: 15, maxHp: 15, speed: 1.2, size: 14, xpValue: 1, damage: 6, color: '#333', hitFlash: 0, spawnAnim: 0.5 }); } } }
        const oc = e.type === 'boss' ? 20 : e.type === 'final_boss' ? 50 : e.type === 'chocolate_chunk' ? 5 : e.type === 'licorice' ? 3 : 1;
        for (let i = 0; i < oc; i++) { if (xpOrbsRef.current.length < MAX_XP_ORBS) { xpOrbsRef.current.push({ x: e.x + (Math.random() - 0.5) * (e.type === 'boss' || e.type === 'final_boss' ? 80 : 20), y: e.y + (Math.random() - 0.5) * (e.type === 'boss' || e.type === 'final_boss' ? 80 : 20), value: e.type === 'final_boss' ? 20 : e.type === 'boss' ? 10 : e.xpValue, size: 6 + (e.type === 'boss' || e.type === 'final_boss' ? 10 : e.xpValue), collectAnim: 0, spawnTime: now }); } }
        const powerUpChance = e.type === 'boss' || e.type === 'final_boss' ? 1.0 : e.type === 'chocolate_chunk' ? 0.01 : 0.0025;
        if (Math.random() < powerUpChance) { const types: PowerUpType[] = ['magnet_burst', 'health', 'freeze']; powerUpsRef.current.push({ x: e.x, y: e.y, type: types[Math.floor(Math.random() * types.length)], size: 14 }); }
        if (e.type === 'final_boss') { endlessModeRef.current = true; shake(50, 1500); flash('#FFD700', 0.5); addParticles(e.x, e.y, '#FFD700', 60, 15, 'burst'); }
        playKill(); addParticles(e.x, e.y, e.color, e.type === 'boss' || e.type === 'final_boss' ? 30 : 8, e.type === 'boss' || e.type === 'final_boss' ? 8 : 4, 'burst');
        if (e.type === 'boss' || e.type === 'final_boss') { shake(30, 800); flash(e.color, 0.4); }
        enemiesKilledRef.current++; setKillCount(enemiesKilledRef.current); return false;
      }
      return true;
    });
    xpOrbsRef.current = xpOrbsRef.current.filter(o => {
      ...
      if (o.magnetized && dist > 5) { const magnetSpeed = 15; o.x += (dx / dist) * magnetSpeed * delta; o.y += (dy / dist) * magnetSpeed * delta; }
      else if (dist < p.magnetRange) { const sp = 5 * (1 - dist / p.magnetRange) + 2; o.x += (dx / dist) * sp * delta; o.y += (dy / dist) * sp * delta; }
      if (dist < PLAYER_SIZE / 2 + o.size) { p.xp += o.value * p.xpMultiplier; playXP(); o.collectAnim = 1; if (Math.random() < 0.3) addParticles(o.x, o.y, '#F472B6', 2, 2, 'spark'); }
      return true;
    });
    powerUpsRef.current = powerUpsRef.current.filter(pu => {
      const dx = p.x - pu.x, dy = p.y - pu.y, dist = Math.hypot(dx, dy);
      if (dist < PLAYER_SIZE / 2 + pu.size) {
        playPowerUp(); addParticles(pu.x, pu.y, POWERUP_CONFIG[pu.type].color, 12, 5, 'ring'); flash(POWERUP_CONFIG[pu.type].color, 0.25);
        if (pu.type === 'magnet_burst') { xpOrbsRef.current.forEach(o => { o.magnetized = true; }); addParticles(p.x, p.y, '#FF00FF', 15, 8, 'ring'); }
        else if (pu.type === 'health') { p.hp = Math.min(p.hp + 20, p.maxHp); addParticles(p.x, p.y, '#FF6B6B', 10, 4, 'glow'); }
        else if (pu.type === 'freeze') { freezeUntilRef.current = now + 10000; addParticles(p.x, p.y, '#00FFFF', 20, 6, 'ring'); enemiesRef.current.forEach(e => { if (Math.random() < 0.3) addParticles(e.x, e.y, '#00FFFF', 3, 2, 'spark'); }); }
        return false;
      }
      return true;
    });
    checkLevelUp(); drawGame(ctx); screenFlashRef.current.intensity *= 0.85;
  ...
  const drawGame = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    powerUpsRef.current.forEach(pu => {
      ...
      if (sx < -20 || sx > CANVAS_WIDTH + 20 || sy < -20 || sy > CANVAS_HEIGHT + 20) return;
      const cfg = POWERUP_CONFIG[pu.type]; const pulse = 1 + Math.sin(now / 150) * 0.15;
      ...
      ctx.fillStyle = '#FFF'; ctx.font = `bold ${Math.floor(pu.size)}px monospace`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(cfg.icon, sx, sy);
      ctx.shadowBlur = 0; ctx.restore();
    });
    const isFrozen = now < freezeUntilRef.current;
    enemiesRef.current.forEach(e => {
      ...
      ctx.scale(spSc, spSc); ctx.globalAlpha = spAl;
      ...
      else if (e.type === 'jellybean') { ctx.beginPath(); ctx.ellipse(0, 0, e.size / 2, e.size / 3, 0, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = 'rgba(255,255,255,0.3)'; ctx.beginPath(); ctx.ellipse(-e.size / 6, -e.size / 8, e.size / 6, e.size / 8, 0, 0, Math.PI * 2); ctx.fill(); }
      else if (e.type === 'licorice') { ctx.fillStyle = e.hitFlash > 0 ? '#FFF' : drawFrozen ? '#88DDFF' : '#1a1a1a'; ctx.beginPath(); ctx.roundRect(-e.size / 2, -e.size / 3, e.size, e.size * 0.66, 4); ctx.fill(); ctx.fillStyle = e.hitFlash > 0 ? '#FFF' : '#FF4444'; ctx.fillRect(-e.size / 2 + 4, -e.size / 3 + 4, 3, e.size * 0.66 - 8); ctx.fillRect(e.size / 2 - 7, -e.size / 3 + 4, 3, e.size * 0.66 - 8); }
      else if (e.type === 'chocolate_chunk') { ctx.beginPath(); ctx.roundRect(-e.size / 2, -e.size / 2, e.size, e.size, 5); ctx.fill(); }
      else if (e.type === 'wind_spirit') { const age = (now - (e.spawnTime || now)) / 1000; const fadeAlpha = age > 8 ? Math.max(0.2, 1 - (age - 8) / 2) : 1; ctx.globalAlpha = 0.6 * fadeAlpha; ctx.strokeStyle = e.color; ctx.lineWidth = 2; for (let i = 0; i < 3; i++) { const phase = now / 200 + i * 2; ctx.beginPath(); ctx.arc(Math.cos(phase) * 4, Math.sin(phase) * 4, e.size / 2 - i * 2, 0, Math.PI * 1.5); ctx.stroke(); } ctx.fillStyle = '#FFF'; ctx.globalAlpha = 0.8 * fadeAlpha; ctx.beginPath(); ctx.arc(0, 0, 4, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1; }
      ...
      else if (e.type === 'final_boss') { const pulse = Math.sin(now / 150) * 0.3 + 0.7; ctx.shadowColor = '#8B0000'; ctx.shadowBlur = 50 * pulse; ctx.fillStyle = '#0a0a0a'; ctx.beginPath(); ctx.arc(0, 0, e.size / 2, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = `rgba(255, 0, 0, ${pulse})`; ctx.beginPath(); ctx.arc(0, 0, 15, 0, Math.PI * 2); ctx.fill(); }
      if (e.maxHp > 20 && e.type !== 'wind_spirit') { const bw = e.type === 'boss' || e.type === 'final_boss' ? e.size * 1.5 : e.size * 1.2; const hp = e.hp / e.maxHp; ctx.fillStyle = '#333'; ctx.fillRect(-bw / 2, -e.size / 2 - 12, bw, e.type === 'boss' || e.type === 'final_boss' ? 6 : 4); ctx.fillStyle = hp > 0.5 ? '#4ADE80' : hp > 0.25 ? '#FBBF24' : '#FF6B6B'; ctx.fillRect(-bw / 2, -e.size / 2 - 12, bw * hp, e.type === 'boss' || e.type === 'final_boss' ? 6 : 4); }
      ctx.restore();
    });
    projectileTrailsRef.current.forEach(t => { const sx = t.x - cam.x, sy = t.y - cam.y; if (sx < -10 || sx > CANVAS_WIDTH + 10 || sy < -10 || sy > CANVAS_HEIGHT + 10) return; ctx.globalAlpha = t.life * 0.5; ctx.fillStyle = t.color; ctx.beginPath(); ctx.arc(sx, sy, 2 * t.life, 0, Math.PI * 2); ctx.fill(); });
    ctx.globalAlpha = 1;
    projectilesRef.current.forEach(pr => { const sx = pr.x - cam.x, sy = pr.y - cam.y; if (sx < -20 || sx > CANVAS_WIDTH + 20 || sy < -20 || sy > CANVAS_HEIGHT + 20) return; ctx.fillStyle = pr.color; ctx.shadowColor = pr.color; ctx.shadowBlur = 8; ctx.beginPath(); ctx.arc(sx, sy, pr.size, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; });
    const psx = p.x - cam.x, psy = p.y - cam.y;
    weaponsRef.current.forEach(w => {
      if (w.type === 'orbiting_donuts') { const cnt = 2 + Math.floor(w.level / 2), rad = 50 + w.level * 5; for (let i = 0; i < cnt; i++) { const a = (w.angle || 0) + (i / cnt) * Math.PI * 2; const ox = psx + Math.cos(a) * rad, oy = psy + Math.sin(a) * rad; ctx.shadowColor = '#F472B6'; ctx.shadowBlur = 10; if (donutImageRef.current && donutLoadedRef.current) { ctx.drawImage(donutImageRef.current, ox - 12, oy - 12, 24, 24); } else { ctx.fillStyle = '#F472B6'; ctx.beginPath(); ctx.arc(ox, oy, 12, 0, Math.PI * 2); ctx.fill(); } ctx.shadowBlur = 0; } }
      else if (w.type === 'frosting_ring') { const rad = 40 + w.level * 8; ctx.strokeStyle = `rgba(96,165,250,${0.3 + Math.sin((w.angle || 0) * 2) * 0.2})`; ctx.lineWidth = 8 + w.level * 2; ctx.shadowColor = '#60A5FA'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(psx, psy, rad, 0, Math.PI * 2); ctx.stroke(); ctx.shadowBlur = 0; }
    });
    ...
    ctx.shadowColor = isTranscendent ? '#FFD700' : '#F472B6'; ctx.shadowBlur = isTranscendent ? 30 : 20;
    if (pfpImageRef.current && pfpLoadedRef.current) { ctx.beginPath(); ctx.arc(0, 0, rad, 0, Math.PI * 2); ctx.closePath(); ctx.clip(); ctx.drawImage(pfpImageRef.current, -rad, -rad, PLAYER_SIZE, PLAYER_SIZE); ctx.restore(); ctx.save(); ctx.translate(psx, psy); if (inv && Math.floor(frameCountRef.current / 4) % 2 === 0) ctx.globalAlpha = 0.5; ctx.strokeStyle = isTranscendent ? '#FFD700' : '#F472B6'; ctx.lineWidth = isTranscendent ? 4 : 3; ctx.shadowColor = isTranscendent ? '#FFD700' : '#F472B6'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(0, 0, rad, 0, Math.PI * 2); ctx.stroke(); }
    else { ctx.fillStyle = isTranscendent ? '#FFD700' : '#F472B6'; ctx.beginPath(); ctx.arc(0, 0, rad, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = '#0d0d0d'; ctx.beginPath(); ctx.arc(0, 0, rad / 3, 0, Math.PI * 2); ctx.fill(); }
    ctx.restore();
    particlesRef.current = particlesRef.current.filter(pt => { pt.x += pt.vx; pt.y += pt.vy; pt.vx *= 0.95; pt.vy *= 0.95; pt.life -= 0.03; if (pt.life <= 0) return false; const sx = pt.x - cam.x, sy = pt.y - cam.y; if (sx < -20 || sx > CANVAS_WIDTH + 20 || sy < -20 || sy > CANVAS_HEIGHT + 20) return false; ctx.globalAlpha = pt.life; ctx.fillStyle = pt.color; if (pt.type === 'ring') { ctx.strokeStyle = pt.color; ctx.lineWidth = 2 * pt.life; ctx.beginPath(); ctx.arc(sx, sy, pt.size * (2 - pt.life), 0, Math.PI * 2); ctx.stroke(); } else if (pt.type === 'burst') { ctx.shadowColor = pt.color; ctx.shadowBlur = 5; ctx.beginPath(); ctx.arc(sx, sy, pt.size * pt.life, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; } else { ctx.beginPath(); ctx.arc(sx, sy, pt.size * pt.life, 0, Math.PI * 2); ctx.fill(); } return true; });
    ctx.globalAlpha = 1;
    damageNumbersRef.current = damageNumbersRef.current.filter(d => { d.y += d.vy; d.vy *= 0.95; d.life -= 0.025; if (d.life <= 0) return false; const sx = d.x - cam.x, sy = d.y - cam.y; if (sx < -50 || sx > CANVAS_WIDTH + 50 || sy < -50 || sy > CANVAS_HEIGHT + 50) return false; ctx.globalAlpha = d.life; ctx.fillStyle = d.isCrit ? '#FFD700' : '#FFF'; ctx.font = d.isCrit ? 'bold 18px monospace' : 'bold 14px monospace'; ctx.textAlign = 'center'; ctx.fillText(d.value.toString(), sx, sy); return true; });
    ctx.globalAlpha = 1;
    if (screenFlashRef.current.intensity > 0.01) { ctx.fillStyle = screenFlashRef.current.color; ctx.globalAlpha = screenFlashRef.current.intensity; ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT); ctx.globalAlpha = 1; }
    const gt = Math.floor((now - gameStartTimeRef.current - pausedTimeRef.current) / 1000);
    const m = Math.floor(gt / 60), s = gt % 60;
    ctx.fillStyle = '#FFF'; ctx.font = 'bold 18px monospace'; ctx.textAlign = 'center'; ctx.fillText(`${m}:${s.toString().padStart(2, '0')}`, CANVAS_WIDTH / 2, 28);
    ctx.font = '10px monospace'; ctx.fillStyle = isTranscendent ? '#FFD700' : '#F472B6'; ctx.fillText(isTranscendent ? `LV ${p.level} ★` : `LV ${p.level}`, CANVAS_WIDTH / 2, 44);
    let indicatorY = 58;
    const freezeRemaining = Math.max(0, (freezeUntilRef.current - now) / 1000);
    if (freezeRemaining > 0) { ctx.fillStyle = '#00FFFF'; ctx.font = 'bold 11px monospace'; ctx.fillText(`❄ FROZEN ${freezeRemaining.toFixed(1)}s`, CANVAS_WIDTH / 2, indicatorY); indicatorY += 14; }
    if (isTranscendent) { ctx.fillStyle = '#FFD700'; ctx.font = 'bold 10px monospace'; ctx.fillText('★ TRANSCENDENT ★', CANVAS_WIDTH / 2, indicatorY); indicatorY += 14; }
    if (endlessModeRef.current) { ctx.fillStyle = '#FFD700'; ctx.font = 'bold 10px monospace'; const diffPercent = Math.round((endlessDifficultyRef.current - 1) * 100); ctx.fillText(`∞ ENDLESS MODE +${diffPercent}%`, CANVAS_WIDTH / 2, indicatorY); indicatorY += 14; }
    ...
    if (isMusicOn && gt > 0) { ctx.fillStyle = currentStyle === 0 ? '#F472B6' : currentStyle === 1 ? '#60A5FA' : currentStyle === 2 ? '#4ADE80' : currentStyle === 3 ? '#A78BFA' : '#FF6B6B'; ctx.font = '9px monospace'; ctx.globalAlpha = 0.7; ctx.fillText(musicStyles[currentStyle], CANVAS_WIDTH / 2, indicatorY); ctx.globalAlpha = 1; }
    if (bossWarningRef.current && !bossSpawnedRef.current) { ctx.fillStyle = '#FF1744'; ctx.font = 'bold 14px monospace'; ctx.fillText('⚠ BOSS INCOMING ⚠', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 50); }
    if (finalBossWarningRef.current && !finalBossSpawnedRef.current) { ctx.fillStyle = '#000'; ctx.font = 'bold 16px monospace'; ctx.fillText('☠ FINAL BOSS ☠', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 50); }
    if (gt >= 300) { const timeSinceHorde = (now - lastWindHordeRef.current) / 1000; if (timeSinceHorde > 40 && timeSinceHorde < 45) { ctx.fillStyle = '#87CEEB'; ctx.font = 'bold 12px monospace'; ctx.fillText('~ WIND SPIRITS APPROACHING ~', CANVAS_WIDTH / 2, 72); } }
    ...
    ctx.fillStyle = '#1a1a1a'; ctx.fillRect(15, CANVAS_HEIGHT - 35, 100, 8);
    ctx.fillStyle = hp > 0.5 ? '#4ADE80' : hp > 0.25 ? '#FBBF24' : '#FF6B6B'; ctx.fillRect(15, CANVAS_HEIGHT - 35, 100 * hp, 8);
    ctx.fillStyle = '#666'; ctx.font = '9px monospace'; ctx.fillText(`${Math.ceil(p.hp)}/${p.maxHp}`, 15, CANVAS_HEIGHT - 40);
    ctx.fillStyle = '#1a1a1a'; ctx.fillRect(15, CANVAS_HEIGHT - 22, CANVAS_WIDTH - 30, 6);
    ctx.fillStyle = '#F472B6'; ctx.fillRect(15, CANVAS_HEIGHT - 22, (CANVAS_WIDTH - 30) * (p.xp / p.xpToLevel), 6);
    ctx.textAlign = 'right';
    weaponsRef.current.forEach((w, i) => { const c = WEAPON_CONFIG[w.type]; ctx.font = 'bold 16px monospace'; ctx.fillStyle = w.level > 8 ? '#FFD700' : c.color; ctx.fillText(c.icon, CANVAS_WIDTH - 15 - i * 26, 28); ctx.font = '8px monospace'; ctx.fillStyle = w.level > 8 ? '#FFD700' : '#FFF'; ctx.fillText(w.level.toString(), CANVAS_WIDTH - 10 - i * 26, 38); });
    gadgetsRef.current.forEach((g, i) => { const c = GADGET_CONFIG[g.type]; ctx.font = 'bold 14px monospace'; ctx.fillStyle = c.color; ctx.fillText(c.icon, CANVAS_WIDTH - 15 - i * 24, 56); ctx.font = '7px monospace'; ctx.fillStyle = '#888'; ctx.fillText('×' + g.stacks.toString(), CANVAS_WIDTH - 10 - i * 24, 65); });
    if (weaponsRef.current.length > 0 || gadgetsRef.current.length > 0) { ctx.fillStyle = 'rgba(255,255,255,0.3)'; ctx.font = '8px monospace'; ctx.textAlign = 'right'; ctx.fillText('tap items ▶', CANVAS_WIDTH - 15, 78); }
    const j = joystickRef.current;
    if (j.active) { ctx.strokeStyle = 'rgba(255,255,255,0.3)'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(j.startX, j.startY, 50, 0, Math.PI * 2); ctx.stroke(); const dx = j.currentX - j.startX, dy = j.currentY - j.startY; const dist = Math.min(Math.hypot(dx, dy), 50); const a = Math.atan2(dy, dx); ctx.fillStyle = 'rgba(244,114,182,0.6)'; ctx.beginPath(); ctx.arc(j.startX + Math.cos(a) * dist, j.startY + Math.sin(a) * dist, 20, 0, Math.PI * 2); ctx.fill(); }
  }, [isMusicOn, isTranscendent]);
  ...
  const startGame = useCallback(() => {
    ...
    gadgetsRef.current = []; enemiesRef.current = []; projectilesRef.current = []; xpOrbsRef.current = []; particlesRef.current = []; damageNumbersRef.current = []; trailPointsRef.current = []; projectileTrailsRef.current = []; powerUpsRef.current = [];
    enemyGridRef.current.clear(); freezeUntilRef.current = 0; screenFlashRef.current = { intensity: 0, color: '#FFF' }; lastWindHordeRef.current = 0;
    ...
    screenShakeRef.current = { intensity: 0, duration: 0, startTime: 0 }; enemiesKilledRef.current = 0; moveInputRef.current = { x: 0, y: 0 };
    joystickRef.current = { active: false, startX: 0, startY: 0, currentX: 0, currentY: 0 }; isPausedRef.current = false;
    bossSpawnedRef.current = false; bossWarningRef.current = false; finalBossSpawnedRef.current = false; finalBossWarningRef.current = false; endlessModeRef.current = false; endlessDifficultyRef.current = 1.0;
    xpCollectedRef.current = 0; damageDealtRef.current = 0; damageTakenRef.current = 0; powerUpsCollectedRef.current = 0; bossesDefeatedRef.current = 0;
    setRerollsLeft(2); setBansLeft(1); setBannedUpgrades([]); setBanMode(false); setScore(0); setPlayerLevel(1); setSurvivalTime(0); setKillCount(0); setGameState("playing"); setShowWeaponMenu(false); setShowGadgetInfo(false); setIsTranscendent(false);
  ...
  const recordEntryAndStartGame = useCallback(async (hash: string) => {
    ...
    try {
      const res = await fetch("/api/games/donut-survivors/free-entry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fid: context.user.fid, walletAddress: address, username: context.user.username, displayName: context.user.displayName, pfpUrl: context.user.pfpUrl, txHash: hash }) });
      const data = await res.json();
      if (data.success) { setCurrentEntryId(data.entryId); currentEntryIdRef.current = data.entryId; currentFidRef.current = context.user.fid; setPlayState('idle'); resetWrite(); startGame(); }
      else { setErrorMessage(data.error || "Failed to record entry"); setPlayState('error'); }
    } catch (error) { console.error("Failed to record entry:", error); setErrorMessage("Failed to record entry"); setPlayState('error'); }
  }, [context?.user, address, resetWrite, startGame]);

  const handlePlay = useCallback(async () => {
    if (!address || !context?.user?.fid) { setErrorMessage("Please connect your wallet"); return; }
    setErrorMessage(null); setPlayState('confirming');
    writeContract({ address: FREE_ARCADE_CONTRACT, abi: FREE_ARCADE_ABI, functionName: "play", args: ["donut-survivors"] });
  }, [address, context?.user?.fid, writeContract]);

  useEffect(() => { if (isConfirmed && txHash && playState === 'confirming') recordEntryAndStartGame(txHash); }, [isConfirmed, txHash, playState, recordEntryAndStartGame]);
  useEffect(() => { if (writeError) { setPlayState('error'); setErrorMessage(writeError.message?.includes("User rejected") ? "Transaction rejected" : "Transaction failed"); } }, [writeError]);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (dist > 8) { const nd = Math.min((dist - 8) / 42, 1), cd = Math.pow(nd, 0.8); moveInputRef.current = { x: (dx / dist) * cd, y: (dy / dist) * cd }; }
    else moveInputRef.current = { x: 0, y: 0 };
  }, []);
  
  const handlePointerUp = useCallback((e: React.PointerEvent) => { try { (e.target as HTMLElement).releasePointerCapture(e.pointerId); } catch {} joystickRef.current.active = false; moveInputRef.current = { x: 0, y: 0 }; }, []);
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => { keys.add(e.code); update(); if ((e.code === 'Tab' || e.code === 'KeyE') && gameState === 'playing') { e.preventDefault(); openEquip(); } if (e.code === 'Escape' && gameState === 'equipment') closeEquip(); };
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.textAlign = 'center'; ctx.shadowColor = '#F472B6'; ctx.shadowBlur = 30; ctx.fillStyle = '#FFF'; ctx.font = 'bold 22px monospace'; ctx.fillText('DONUT SURVIVORS', CANVAS_WIDTH / 2, 80); ctx.shadowBlur = 0;
      const py = 155 + Math.sin(t * 1.5) * 8, pr = 28;
      ctx.save(); ctx.shadowColor = '#F472B6'; ctx.shadowBlur = 25;
      if (pfpImageRef.current && pfpLoadedRef.current) { ctx.beginPath(); ctx.arc(CANVAS_WIDTH / 2, py, pr, 0, Math.PI * 2); ctx.closePath(); ctx.clip(); ctx.drawImage(pfpImageRef.current, CANVAS_WIDTH / 2 - pr, py - pr, pr * 2, pr * 2); ctx.restore(); ctx.strokeStyle = '#F472B6'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(CANVAS_WIDTH / 2, py, pr, 0, Math.PI * 2); ctx.stroke(); }
      ...
      for (let i = 0; i < 4; i++) { const a = t * 1.5 + (i / 4) * Math.PI * 2; const ox = CANVAS_WIDTH / 2 + Math.cos(a) * 55, oy = py + Math.sin(a) * 55 * 0.6; ctx.fillStyle = '#F472B6'; ctx.shadowColor = '#F472B6'; ctx.shadowBlur = 8; ctx.beginPath(); ctx.arc(ox, oy, 6, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; }
    ...
    <main className="flex h-screen w-screen justify-center overflow-hidden bg-black font-mono text-white select-none">
      <style>{`
        ...
        * { -webkit-tap-highlight-color: transparent !important; }
        main, main * { user-select: none !important; -webkit-user-select: none !important; }
              ...
              <div className="absolute inset-0 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center p-4 z-30 rounded-2xl">
                ...
                <div className="flex flex-col gap-2 w-full max-w-[290px]">
                  {upgradeOptions.map((opt, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => { if (banMode) { setBansLeft(b => b - 1); setBannedUpgrades(prev => [...prev, opt.type === 'weapon' ? `weapon:${opt.weaponType}` : `gadget:${opt.gadgetType}`]); setBanMode(false); genUpgrades(); } else applyUpgrade(opt); }} className={`flex items-center gap-3 p-3 bg-zinc-900/80 border rounded-lg transition-all active:scale-[0.98] ${banMode ? 'border-red-500/70 hover:bg-red-500/15' : opt.isTranscendent ? 'border-yellow-500/70 hover:border-yellow-400' : opt.type === 'weapon' ? 'border-zinc-700/50 hover:border-pink-500/70' : 'border-zinc-700/50 hover:border-blue-500/70'}`}>
                        <span className="text-2xl font-bold w-8 text-center" style={{ color: banMode ? '#FF6B6B' : opt.isTranscendent ? '#FFD700' : cfg?.color || '#FFF' }}>{opt.icon}</span>
                        <div className="flex-1 text-left">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-white text-sm">{opt.title}</span>
                            <span className={`text-[8px] uppercase tracking-wider px-1.5 py-0.5 rounded ${banMode ? 'bg-red-500/20 text-red-400' : opt.isTranscendent ? 'bg-yellow-500/20 text-yellow-400' : opt.type === 'weapon' ? 'bg-pink-500/15 text-pink-400' : 'bg-blue-500/15 text-blue-400'}`}>{banMode ? 'Ban' : opt.isTranscendent ? 'Transcend' : opt.type}</span>
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) closeEquip(); }} onTouchEnd={(e) => { if (e.target === e.currentTarget) { e.preventDefault(); closeEquip(); } }}>
          <div className="w-full max-w-[360px] bg-black/98 backdrop-blur-sm flex flex-col rounded-2xl overflow-hidden border border-zinc-700" style={{ maxHeight: '80vh' }} onClick={(e) => e.stopPropagation()}>
            <div className="flex-shrink-0 p-3 border-b border-zinc-800 relative">
              ...
              {isTranscendent && <div className="text-[10px] text-yellow-400 text-center mt-1">★ Transcendent Mode Active ★</div>}
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-3 scrollable-area" style={{ WebkitOverflowScrolling: 'touch' }}>
              <div>
                ...
                <div className="space-y-2">
                  {equipmentData.weapons.map((w, i) => { const cfg = WEAPON_CONFIG[w.type], stats = WEAPON_STATS[w.type]; const dmg = cfg.baseDamage * (1 + (w.level - 1) * 0.3) * equipmentData.player.damage; const cdRed = 1 - (equipmentData.player.cooldownReduction || 0); const cd = cfg.baseCooldown * Math.pow(0.9, w.level - 1) * cdRed; const isTranscendentWeapon = w.level > 8; return (<div key={i} className={`bg-zinc-900/80 border rounded-lg p-2 ${isTranscendentWeapon ? 'border-yellow-500/50' : 'border-zinc-800'}`}><div className="flex items-center gap-2 mb-1"><span className="text-xl" style={{ color: isTranscendentWeapon ? '#FFD700' : cfg.color }}>{cfg.icon}</span><span className="font-medium text-white text-sm flex-1">{cfg.name}</span><span className={`text-[10px] px-1.5 py-0.5 rounded ${isTranscendentWeapon ? 'bg-yellow-500/20 text-yellow-400' : 'bg-pink-500/20 text-pink-400'}`}>LV {w.level}{isTranscendentWeapon ? ' ★' : ''}</span></div><div className="grid grid-cols-3 gap-1 text-[9px]"><div><span className="text-zinc-500">DMG:</span> <span className="text-zinc-300">{Math.floor(dmg)}</span></div><div><span className="text-zinc-500">CD:</span> <span className="text-zinc-300">{Math.floor(cd)}ms</span></div><div><span className="text-zinc-500">{stats.stat}:</span> <span className="text-zinc-300 text-[8px]">{stats.perLevel}</span></div></div></div>); })}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {equipmentData.gadgets.map((g, i) => { const cfg = GADGET_CONFIG[g.type]; return (<div key={i} className="bg-zinc-900/80 border border-zinc-800 rounded-lg p-2"><div className="flex items-center gap-2"><span className="text-xl" style={{ color: cfg.color }}>{cfg.icon}</span><div className="flex-1"><div className="flex items-center gap-2"><span className="font-medium text-white text-sm">{cfg.name}</span><span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded">×{g.stacks}</span></div><div className="text-[9px] text-green-400">{getGadgetBonus(g.type, g.stacks)}</div></div></div></div>); })}
