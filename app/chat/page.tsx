...
export default function ChatPage() {
  ...
  return (
    <main className="flex h-[100dvh] w-screen justify-center overflow-hidden bg-black font-mono text-white">
      <div className="relative flex h-full w-full max-w-[520px] flex-1 flex-col overflow-hidden rounded-[28px] bg-black px-2 shadow-inner" style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 8px)", paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto space-y-2 pb-2 min-h-0 scrollbar-hide">
            {messagesLoading ? (
              ...
              <>
                ...
                <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-xl p-2">
                  <input
                    ...
                    placeholder={rateLimitBanRemaining > 0 ? `Banned for ${formatBanTime(rateLimitBanRemaining)}...` : cooldownRemaining > 0 ? `Wait ${cooldownRemaining}s...` : "Type a message..."}
                    disabled={isPending || isConfirming || cooldownRemaining > 0 || rateLimitBanRemaining > 0 || isVerifying}
                  ...
                  <button onClick={handleSendMessage} disabled={!message.trim() || isPending || isConfirming || cooldownRemaining > 0 || rateLimitBanRemaining > 0 || isVerifying} className="flex items-center justify-center w-9 h-9 rounded-lg bg-white text-black disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-200 transition-colors">
                    {rateLimitBanRemaining > 0 ? <Timer className="w-4 h-4 text-red-500" /> : cooldownRemaining > 0 ? <span className="text-xs font-bold">{cooldownRemaining}</span> : isVerifying ? <span className="text-xs font-bold">...</span> : <Send className="w-4 h-4" />}
                  </button>
                </div>
