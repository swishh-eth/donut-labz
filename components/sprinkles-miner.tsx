...
const DonutCoin = ({ className = "w-4 h-4" }: { className?: string }) => (
  <span className={`${className} rounded-full overflow-hidden inline-flex items-center justify-center flex-shrink-0`}>
    <img src="/coins/donut_logo.png" alt="DONUT" className="w-full h-full object-cover scale-[1.55]" />
  </span>
);
  ...
  useEffect(() => {
    ...
    if (receipt.status === "success" || receipt.status === "reverted") {
      ...
      if (receipt.status === "success" && txType === "mine" && address) {
        ...
        console.log('Mine success! Preparing to record glaze:', { txHashToRecord, paidAmount });
          ...
          <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
            ...
            >
              <div className="rounded-full bg-black p-0.5 spin-slow">
                <Avatar className="h-24 w-24 border-2 border-green-400/50">
                  <AvatarImage
              ...
              <div className="mt-2 text-center">
                <div className="font-bold text-green-400 text-sm drop-shadow-lg">{occupantDisplay.primary}</div>
                <div className="text-[10px] text-green-400/70 drop-shadow-lg">{formatAddress(minerAddress)}</div>
              </div>
